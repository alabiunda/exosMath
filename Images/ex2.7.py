...
image_sortie=np.copy(image) # ici on recopie simplement l'image d'entrée dans celle de sortie

for tab in range(nb_tab):
    for lig in range(nb_lignes):
        for col in range(nb_colonnes):
            image_sortie[lig][col][tab]=int(math.floor(image[lig][col][0]*0.2126+image[lig][col][1]*0.7152+image[lig][col][2]*0.0722))
